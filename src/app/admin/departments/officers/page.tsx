    ...
    <AppShell user={{ name: user.name, role: user.role as any }}>
      <div className="space-y-6">
        <PageHeader
          ...
          description="All staff members with the Department Officer role"
          actions={
        ...
        <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
          {loading ? (
            ...
            <EmptyState
              icon={Users}
              title="No Department Officers"
              description="No staff members with the Department Officer role exist yet."
            />
          ) : (
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-slate-100 bg-slate-50">
                    {['Name', 'Email', 'Department', 'Status', 'Joined', 'Actions'].map((h) => (
                      <th
                        key={h}
